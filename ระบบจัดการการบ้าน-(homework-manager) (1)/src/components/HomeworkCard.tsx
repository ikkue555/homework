    ...
    <div 
      className={`bg-white dark:bg-slate-900 rounded-3xl border transition-all duration-300 overflow-hidden shadow-xs card-interactive flex flex-col justify-between ${
        homework.completed
          ...
          : isOverdue
          ? 'border-rose-300/90 dark:border-rose-800/90 bg-rose-50/15 dark:bg-rose-950/15'
          : isDueToday
          ? 'border-amber-300/90 dark:border-amber-800/90 bg-amber-50/15 dark:bg-amber-950/15'
    ...
    >
      <div className="p-3.5 sm:p-5 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          {/* Prominent Subject Tag & Meta Group */}
          <div className="flex flex-wrap items-center gap-1.5 min-w-0">
            {/* Prominent Subject Badge */}
            <span className="inline-flex items-center space-x-1.5 text-xs sm:text-sm font-bold font-heading px-3 py-1 rounded-xl bg-sky-100 dark:bg-sky-950 text-sky-900 dark:text-sky-100 border border-sky-300 dark:border-sky-700 shadow-xs shrink-0">
              <BookOpen className="w-3.5 h-3.5 text-sky-600 dark:text-sky-400 shrink-0" />
              <span className="truncate max-w-[170px] sm:max-w-[220px]">{homework.subject}</span>
            </span>

            {/* Work Type Badge */}
            <span 
              className={`text-[10px] sm:text-[11px] font-medium px-2 py-0.5 rounded-lg flex items-center space-x-1 shrink-0 ${
            ...
            {homework.priority === 'ด่วนที่สุด' && (
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-lg bg-rose-100 dark:bg-rose-950 text-rose-700 dark:text-rose-300 border border-rose-200/80 dark:border-rose-800 shrink-0 animate-pulse">
                🔥 ด่วน
              </span>
            )}
          </div>
        ...
        {/* Prominently Highlighted Due Date & Countdown Status Box */}
        <div 
          className={`flex flex-wrap items-center justify-between gap-2 p-2.5 sm:px-3.5 sm:py-2.5 rounded-2xl border text-xs mb-3 transition-colors ${
            homework.completed
              ...
              : 'bg-sky-50/60 dark:bg-slate-800/90 border-sky-100 dark:border-slate-700'
          }`}
        >
          ...
          <div className="shrink-0">
            {homework.completed ? (
              <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-xl text-xs font-bold font-heading bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                <Sparkles className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                <span>เสร็จแล้ว</span>
              </span>
            ) : hasNoDueDate ? (
              <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-xl text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                <Clock className="w-3.5 h-3.5 text-slate-400" />
                <span>ไม่มีกำหนด</span>
              </span>
            ) : isOverdue ? (
              <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-xl text-xs font-bold font-heading bg-rose-600 text-white shadow-xs animate-pulse">
                <AlertCircle className="w-3.5 h-3.5 text-white" />
                <span>เลยกำหนด {Math.abs(diffDays)} วัน</span>
              </span>
            ) : isDueToday ? (
              <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-xl text-xs font-bold font-heading bg-amber-500 text-white shadow-xs">
                <Clock className="w-3.5 h-3.5 text-white" />
                <span>ส่งวันนี้!</span>
              </span>
            ) : (
              <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-xl text-xs font-bold font-heading bg-sky-100 dark:bg-sky-950 text-sky-800 dark:text-sky-300 border border-sky-200 dark:border-sky-800">
