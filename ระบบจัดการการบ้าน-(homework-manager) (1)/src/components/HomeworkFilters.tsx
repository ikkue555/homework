...
import { Search, RotateCcw, Calendar, BookOpen, Users, Tag } from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-xs mb-6 space-y-4">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-12 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:bg-white focus:border-sky-500 transition-all"
          />
          {filters.searchQuery && (
            <button
        ...
        <div className="flex items-center space-x-3 justify-between md:justify-end">
          <div className="flex items-center space-x-1.5 text-xs text-slate-500">
            ...
            <select
              ...
              className="bg-slate-50 border border-slate-200 text-slate-700 text-xs rounded-xl px-2.5 py-2 font-normal focus:outline-none focus:ring-2 focus:ring-sky-500/20"
            >
          ...
          <span className="text-xs font-normal px-2.5 py-1.5 bg-slate-100 text-slate-700 rounded-xl border border-slate-200/60 whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-3 border-t border-slate-100">
        {/* Filter by Subject */}
        <div>
          <label className="text-xs font-normal text-slate-500 mb-1 flex items-center space-x-1">
          ...
          <select
            ...
            className="w-full bg-slate-50 border border-slate-200 text-slate-700 text-xs rounded-xl px-3 py-2 font-normal focus:outline-none focus:ring-2 focus:ring-sky-500/20"
          >
      ...
      {hasActiveFilters && (
        <div className="pt-2 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            ...
            {filters.subject && (
              <span className="bg-sky-50 text-sky-700 border border-sky-100 px-2.5 py-0.5 rounded-full font-normal">
                วิชา: {filters.subject}
              </span>
            )}
            {filters.dueDateFilter !== 'all' && (
              <span className="bg-blue-50 text-blue-700 border border-blue-100 px-2.5 py-0.5 rounded-full font-normal">
                วันส่ง: {
            ...
            {filters.type && (
              <span className="bg-indigo-50 text-indigo-700 border border-indigo-100 px-2.5 py-0.5 rounded-full font-normal">
                ประเภท: {filters.type}
              </span>
            )}
            {filters.workType !== 'all' && (
              <span className="bg-teal-50 text-teal-700 border border-teal-100 px-2.5 py-0.5 rounded-full font-normal">
          ...
          <button
            onClick={handleReset}
            className="inline-flex items-center space-x-1 text-xs text-rose-600 hover:text-rose-700 font-medium cursor-pointer py-1 px-2 rounded-lg hover:bg-rose-50 transition-colors"
          >
