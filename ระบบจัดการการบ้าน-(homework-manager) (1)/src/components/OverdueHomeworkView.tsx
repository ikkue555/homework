...
export const OverdueHomeworkView: React.FC<OverdueHomeworkViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Urgency Banner */}
      <div className="bg-gradient-to-r from-rose-600 to-red-700 text-white p-6 sm:p-8 rounded-3xl shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-white/20 text-rose-100 text-xs font-semibold mb-3">
              <AlertTriangle className="w-3.5 h-3.5 text-rose-200" />
              <span>ต้องการการจัดการด่วน</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold font-heading tracking-tight">
              การบ้านที่เลยกำหนดส่งแล้ว
            </h2>
            <p className="text-sm text-rose-100 mt-1 max-w-xl">
              มีงานที่เกินวันส่งจำนวน <strong className="text-white text-base">{overdueHomeworks.length}</strong> รายการ รีบเร่งทำหรือรีบอัปเดตความคืบหน้ากันเลย!
            </p>
          </div>
